...
# 3. Načtení databáze časopisů Q1/Q2
def load_q1_q2_journals(filename="q1_q2_journals.csv"):
    try:
...
# 4. Uložení výsledků lokálně
def save_to_local(df, filename="filtered_pubmed_articles.csv"):
    save_path = os.path.join(os.getcwd(), filename)
    df.to_csv(save_path, index=False)
    print(f"Výsledky uloženy: {save_path}")

# 5. Odeslání e-mailu přes Gmail SMTP s GitHub Secrets
def send_email():
    ...
    with open("filtered_pubmed_articles.csv", "rb") as f:
